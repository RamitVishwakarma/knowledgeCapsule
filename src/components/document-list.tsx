...
import { useAppStore, Document } from "@/store/appStore";
import {
  Plus,
  Play,
  FileText,
...
export function DocumentList() {
  ...
  const topicDocs = documents.filter((d) => d.topicId === selectedTopicId && !d.archived);
  ...
  const handleSubmit = () => {
    ...
    addDocument({ ...formData, topicId: selectedTopicId! });
  ...
  // Unused variable suppression — selectTopic is available for future use
  void selectTopic;
    ...
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="sticky top-0 bg-background/95 backdrop-blur-sm z-10 px-6 py-4 border-b border-border">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-foreground">{currentTopic?.name}</h2>
            <p className="text-muted-foreground text-[13px]">
              {topicDocs.length} recording{topicDocs.length !== 1 ? "s" : ""}
            </p>
          </div>
          <button
      ...
      <div className="p-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, longDescription: e.target.value }))
                  }
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="px-5 py-2.5 bg-primary text-primary-foreground rounded-lg text-[14px] hover:opacity-90"
                >
                  Save recording
                </button>
                <button
        ...
        {/* Document Cards */}
        {topicDocs.length === 0 && !showAddForm ? (
          ...
          <div className="space-y-3">
            {topicDocs.map((doc) => (
              <DocumentCard key={doc.id} doc={doc} onClick={() => selectDocument(doc.id)} />
            ))}
          </div>
        )}
      </div>
...
function ArchivedList() {
  const { documents, topics, selectDocument, restoreDocument, permanentlyDeleteDocument } =
    useAppStore();
      ...
      <div className="sticky top-0 bg-background/95 backdrop-blur-sm z-10 px-6 py-4 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-[13px]">
              {archivedDocs.length} archived recording{archivedDocs.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
      </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="px-2 py-0.5 rounded text-[11px] bg-gray-100 text-gray-500 shrink-0">
                        {getTopicName(doc.topicId)}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-[12px] text-primary border border-primary/20 rounded-lg hover:bg-primary/5 flex items-center gap-1.5 transition-colors"
                      >
                      ...
                      {deleteConfirmId === doc.id ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1.5 text-[12px] bg-destructive text-destructive-foreground rounded-lg"
                          >
                            Yes, delete
                          </button>
                          <button
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3">
            ...
            <div
              className={`px-2.5 py-1 rounded-full text-[11px] flex items-center gap-1 shrink-0 ${status.className}`}
            >
              <StatusIcon
          ...
          {doc.summary && (
            <p className="text-muted-foreground text-[12px] mt-2 line-clamp-2 leading-relaxed">
              {doc.summary.replace(/[#*]/g, "").substring(0, 150)}...
            </p>
          )}
